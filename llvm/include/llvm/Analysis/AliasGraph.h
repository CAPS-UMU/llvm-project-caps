...
// 
// Type aliasing for alias graph analysis
//
using ModuleList = vector<pair<Module *, StringRef>>;
// Mapping module to its file name.
using ModuleNameMap = unordered_map<Module*, StringRef>;
// The set of all functions.
using FuncSet = SmallPtrSet<Function*, 8>;
// Mapping from function name to function.
using NameFuncMap = unordered_map<string, set<size_t>>;
using CallInstSet = SmallPtrSet<CallInst*, 8>;
using CallerMap = DenseMap<Function*, CallInstSet>;
using CalleeMap = DenseMap<CallInst *, FuncSet>;
// Pointer analysis types.
using PointerAnalysisMap = map<Value *, set<Value *>>;
using FuncPointerAnalysisMap = map<Function *, PointerAnalysisMap>;
using FuncAAResultsMap = map<Function *, AAResults *>;
using FuncStructAnalysisMap = map<Function *, PointerAnalysisMap>;
// init-fini analysis    
using StructInfoMap = map<ConstantStruct*, set<Function*>>;
using ModuleInfoMap = map<Module*, StructInfoMap>;
...
public:
    ...
    void writeToDot(raw_ostream &dotFile, std::string nodeName){
        dotFile << nodeName+" [label=\""+nodeName+"\\l| ";
        for(auto it = aliasclass.begin(); it != aliasclass.end(); it++){
            Value *v = *it;
            if(Function *F = dyn_cast<Function>(v)){
            ...
            std::string test = to_string(*v);
            for(char &c : test) if (c == '\"') c = ' ';
            dotFile << test << " ; \\l ";
        }
        dotFile << "\"];\n"; 
    }
...
void analyzeGlobalInitializer(GlobalVariable* GV, list<Value *>&Future_analysis_list,  AliasGraph *aliasCtx);
...
struct GlobalContext {
	...
	/* std::unordered_map<std::string, Function*> GlobalFuncs_old; seems unused */
